...
const RegisterPage = observer(() => {
  ...
  const sendEmailCode = async () => {
    ...
    try {
      // 先检查邮箱是否已注册
      const checkResponse = await http.post('/auth/register/check', { email: formData.email });
      ...
      // 发送验证码
      await http.post('/auth/register/verification-code/send', { email: formData.email });
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="text-center space-y-3">
              <p className="text-sm text-default-600">
                已有账号？{' '}
                <Link href="/login" className="text-transparent bg-gradient-to-r from-primary-600 to-secondary-600 bg-clip-text hover:underline font-medium">
                  立即登录
                </Link>
              </p>
